...
{
  public:
    CIndex(const T_ & original, C_ operace = C_()) : m_seznam(original), m_velikost(original.size()), m_comparator(operace){}
    set<size_t> search(const T_ & hledane) const{
      set<size_t> vysledek;
      size_t velikost = hledane.size();
      if(velikost == 0) { // kdyz hledam prazdny retezec, vratim vsechny pozice
        for(size_t i = 0; i < m_velikost; i++){
          vysledek.emplace(i);
        }
        return vysledek;
      }
      size_t position = 0;
      for(auto it = m_seznam.begin(); it != m_seznam.end(); it++){
        bool found = false;
        auto pozice = it;
        for (auto itHl = hledane.begin(); itHl != hledane.end(); itHl++, pozice++){
          found = false;
          if( (m_comparator(*itHl,*pozice)) || (m_comparator(*pozice,*itHl)) ){
          break;
          }
          found = true;
        }
        if(found){
          vysledek.emplace(position);
          //cout << "nalezeno na pozici: " << position << endl;
        }
        position++;
      }
      return vysledek;
    ...
    void debugPrint()const{
      cout << "seznam o velikost: " << m_velikost << endl;
      for(size_t i = 0; i < m_velikost; i++){
        cout << m_seznam.at(i);
      }
      cout << endl;
...
cout << " *** prepared test completed ***" << endl;
